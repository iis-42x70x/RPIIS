...
<hr>
<h3>Подробное описание алгоритма:</h3>
<p>Пользователь задает количество вершин первого и второго графов, после чего определяет их. Затем пользователем вводится количество ребер каждого из графов. Далее пользователь определяет ребра. По полученным исходным данным строится матрица смежности для каждого из графов. Затем для объединения графов производится объединение вершин первого и второго графов с удалением повторяющихся вершин (если такие имеются). После чего происходит объединение множеств ребер двух графов с удалением потворяющихся ребер (если такие имеются). По полученным данным строится матрица смежности - результат выполнения объединения графов.</p>
<hr>
