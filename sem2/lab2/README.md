...
3. **Генерация перестановок**:
   - Для `SET`: полные перестановки элементов
   - Для `ORSET`: сохраняет порядок, но переставляет вложенные `SET`

4. **Вывод результатов**


Пример работы для `{a,<b,c>}`:
1. `{a,<b,c>}`
2. `{<b,c>,a}` 

`<b,c>` остается неизменным (так как ORSET)
