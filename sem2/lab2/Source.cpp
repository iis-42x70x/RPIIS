...
/*remove_if ... ::isspace перемещает все, условно, встретившиеся пробелы в конец строки;
указатель остается на первом пробеле в конце, erase же удаляет все элемены, начиная с этого указателя до конца строки
(на конец указывает второй аргумент функции erase - simple.end()) */
...
bool isValidSet(const char* line) {
    ...
    for (; *line; ++line) { //пока *line = true, т.е. пока *line != '\0'
        if (*line == ',') {
            if (prev_comma) return false;
            prev_comma = true;
        }
        else if (!isspace(*line)) { //не сбрасывает флаг, если пробельный символ
            prev_comma = false;
        }
        switch (*line) {
        case '{': braces++; break;
...
int readSets(const char* filename, char sets[MAX_SETS][MAX_LINE_LENGTH], int& setCount) {
    ...
    while (file.getline(line, MAX_LINE_LENGTH)) {
        if (setCount >= MAX_SETS) {
            cout << "Слишком много множеств!" << endl;
            return -1;
        }
        if (isValidSet(line)) {
            ...
            cout << "Некорректное множество: " << line << endl;
            return -1;
        }
    }
    return 0;
...
void menu(const char* filename) {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            if (setCount < 2) {
                cout << "Нужно минимум 2 множества!" << endl;
                continue;
            }
            int a, b;
            cout << "Номера множеств: "; cin >> a >> b; cin.ignore();
            if (a < 1 || a > setCount || b < 1 || b > setCount) {
                cout << "Неверные номера!" << endl;
                continue;
            }
            if (a == b) {
                cout << "Зачем вводить одинаковые?" << endl;
                continue;
            }
