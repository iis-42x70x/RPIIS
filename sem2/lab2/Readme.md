...
- `{1,2,3}` → числовые элементы разрешены (ожидается true).
- `{a}` → валидная строка (ожидается true).
- `{a,!b}` → невалидная строка из-за символа ! (ожидается false).
- `a,b}`  → нет открывающей скобки ! (ожидается false).
- `{a,b` → нет закрывающей скобки ! (ожидается false).
- `{a,,b}` → пустой элемент между запятыми ! (ожидается false).
- `{a,b, }` → пустой элемент в конце ! (ожидается false).
#####  Цель:
...
1) Обработка дубликатов (например, `{a,a,b}` → `{a,b}`).
2) Корректность работы с числами и спецсимволами (`{a_b, c-d, 123}`).
#### Работа тестов:
![tests](https://github.com/iis-42x70x/RPIIS/blob/%D0%92%D0%B5%D0%B3%D0%B5%D1%80%D0%B0_%D0%9C/sem2/lab2/%7B6641D2A6-B4C1-487B-ABF5-3683D72AB062%7D.png)
## Источники: 
