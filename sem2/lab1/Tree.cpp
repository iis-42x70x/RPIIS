...
int query(Node* node, int l, int r, int sumAdd) {
    if (node->r < l || node->l > r) {
        return INT_MIN;
    }
    if (l <= node->l && node->r <= r) {
        return node->max + sumAdd;
    }

    sumAdd += node->add;
    return max(query(node->left, l, r, sumAdd), query(node->right, l, r, sumAdd));
}

void collectValues(Node* root, int start, int end, int* output) {
    for (int i = start; i <= end; i++) {
        output[i] = query(root, i, i, 0);
    }
}
